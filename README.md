...
- 名单数据随 git 入库即备份（本地 + GitHub 远端），变更当日提交并 push；恢复用 `git checkout HEAD -- data/` 配合热重载秒级生效。
- 数据更新流程：源文件归档 → 规范化（字段白名单/班级键 N班/标题-文件名一致）→ 校验（`go test` + CI 数据契约）→ 人工复核（班级人数 20-60 区间，异常小班如 14 班 26 人需回源核对）→ 独立提交 + CHANGELOG 条目。
- 真实名单（未成年人个人信息）仅限校内部署；公开发布包 data-sample 为假名示例。删除/更正请求联系学校教务处。
