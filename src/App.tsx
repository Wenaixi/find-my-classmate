...
export function App() {
  ...
  // 视图需要的全部展示派生（结果区段 / 滚动时机 / 禁用态 / 按钮标签 / 提示色 / 加载指示）
  // 由 searchReducer 的 present 单点持有。此前组件分别 import 三个派生函数并另调
  // controller 取 state，界面知识横跨两个 module，且既有测试分别打三个函数、
  // 无任何断言锁住它们对同一状态给出一致的组合。
  ...
  useEffect(() => {
    // 滚动时机与结果区显隐同源：都来自 present 的同一次派生。
    if (!shouldScrollRef.current || !view.scroll) return;
  ...
  function renderResultBody() {
    // 状态到区段的映射由 present 单点持有，组件只按返回值选择 JSX。
    switch (view.section) {
      case "loading":
  ...
  const hasResultSection = view.section !== null;
  return (
          ...
          <form className="search-wrap" data-od-id="search-form" ref={searchWrapRef} onSubmit={submit}>
            ...
            <BorderBeam size="md" colorVariant="colorful" theme="dark" borderRadius={999} duration={2.2} strength={1} brightness={2} saturation={2.2} hueRange={160}>
              <div className="search-track" data-od-id="search-track">
                ...
                <button className="search-send" data-od-id="search-cta" type="submit" disabled={view.busy} aria-label={view.sendLabel}>
