  ...
  return (
    <div className="app-shell" data-od-id="app-shell">
      ...
      <main id="top">
        ...
        {hasResultSection && <section className="results-section" ref={resultsRef} data-od-id="results-section" aria-labelledby="results-title"><div className="results-head"><div><p className="section-kicker">SEARCH OUTPUT</p><h2 id="results-title" data-od-id="results-title">查询结果</h2></div><div className="result-count-block"><span className="result-count" data-od-id="result-count">{total || "--"}</span><span className="result-count-label">MATCHES</span></div></div><ErrorBoundary><Suspense fallback={<div className="result-loading"><span>正在加载结果组件</span></div>}>{renderResultBody()}</Suspense></ErrorBoundary></section>}
