      ...
      <main id="top">
        <section className="hero" data-od-id="hero">
          ...
          <form className="search-wrap" data-od-id="search-form" ref={searchWrapRef} onSubmit={submit}>
            ...
            <div className="status-line" id="status-line" data-od-id="status-feedback" data-state={state} aria-live="polite"><Suspense fallback={null}><StatusOrb state={state} /></Suspense><span>{state === "duplicate" ? COPY[state] + "，先显示前 " + PAGE_SIZE + " 条" : COPY[state]}</span></div>
          </form>
        </section>
        {hasResultSection && <section className="results-section" ref={resultsRef} data-od-id="results-section" aria-labelledby="results-title"><div className="results-head"><div><p className="section-kicker">SEARCH OUTPUT</p><h2 id="results-title" data-od-id="results-title">查询结果</h2></div><div className="result-count-block"><span className="result-count" data-od-id="result-count">{total || "--"}</span><span className="result-count-label">MATCHES</span></div></div><Suspense fallback={<div className="result-loading"><span>正在加载结果组件</span></div>}>{renderResultBody()}</Suspense></section>}
