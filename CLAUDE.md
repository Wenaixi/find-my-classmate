...
- border-beam 1.3.0：只包裹 src/App.tsx 的搜索轨道，使用 size="md"、borderRadius=999（胶囊全边框）、colorVariant="colorful"（彩色光束）、theme="dark"、duration=2.2、strength=1、brightness=1.7、saturation=1.9、hueRange=120；官方源码三层结构（::after 彩带转圈 + ::before 28px 内晕 + bloom 辉光）通过官方参数与 CSS 变量增强：--beam-inner-opacity=2.4、--beam-bloom-opacity=2、关键：blur 被 mask 裁剪无效，须覆盖 ::after padding 加宽彩带环（1px -> 12px）才能真正向内延伸；--beam-bloom-blur=40px（辉光向内扩散）、--beam-glow-brightness=2、--beam-glow-saturate=1.8；全走官方变量，不 hack 结构；输入非空时输入框尾部显示 .search-clear 清空小 x 按钮（type=button 不触发搜索，复用 clear()）；光束无条件常转常亮（默认 active=true），strength 恒为 1（拉满）。搜索轨道为胶囊圆角（999px），常驻彩色环境辉光（box-shadow 蓝+粉双层），focus-within 时增强，虚焦后光晕不消失；移动端保持单行布局。
