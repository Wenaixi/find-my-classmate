...
- border-beam 1.3.0：只包裹 src/App.tsx 的搜索轨道，使用 size="md"、borderRadius=999（胶囊全边框）、colorVariant="colorful"（彩色光束）、theme="dark"、brightness=1.6；光束无条件常转常亮（默认 active=true），idle strength 0.85，loading 时提升至 1。搜索轨道为胶囊圆角（999px），移动端保持单行布局。
