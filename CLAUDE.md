...
- border-beam 1.3.0：只包裹 src/App.tsx 的搜索轨道，使用 size="md"、borderRadius=999（胶囊全边框）、colorVariant="colorful"（彩色光束）、theme="dark"、brightness=1.7、saturation=2、hueRange=120（宽幅彩色流动）、duration=2.4；光束无条件常转常亮（默认 active=true），strength 恒为 1（拉满）。搜索轨道为胶囊圆角（999px），常驻彩色环境辉光（box-shadow 蓝+粉双层），focus-within 时增强，虚焦后光晕不消失；移动端保持单行布局。
